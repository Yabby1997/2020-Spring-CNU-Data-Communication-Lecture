...
public class ChatAppLayer implements BaseLayer {
  ...
  /**/
    private void fragSend(byte[] input, int length) {
        ...
        int i = 0;
        m_sHeader.capp_totlen = intToByte2(length);										//첫 헤더엔 전체 크기?
        m_sHeader.capp_type = (byte) (0x01);

        // 첫번째 전송
        System.arraycopy(input, 0, bytes, 0, 10);										//첫번째 10바이트를 전송
        bytes = objToByte(m_sHeader, bytes, 10);
        this.GetUnderLayer().Send(bytes, bytes.length);

        int maxLen = length / 10;
        m_sHeader.capp_totlen = intToByte2(10);											//중간 부분은 모두 크기가 10으로 잘려있다. 
        m_sHeader.capp_type = (byte)(0x02);												//중간 부분은 타입이 0x02이다.
        for(i = 1; i < maxLen; i++) {
        	waitACK();
        	if(i + 1 == maxLen && length % 10 == 0)										//마지막 데이터인 경우 타입을 바꿔준다. 
        		m_sHeader.capp_type = (byte)(0x03);
        	System.arraycopy(input, 10 * i, bytes, 0, 10);								//이후 자리부터 10개를 bytes에 복사
        	bytes = objToByte(m_sHeader, bytes, 10);
        	this.GetUnderLayer().Send(bytes, bytes.length);
        }

        if (length % 10 != 0) {															//나머지 부분, 즉 마지막 부분
        	waitACK();
            m_sHeader.capp_totlen = intToByte2(length % 10);
        	m_sHeader.capp_type = (byte) (0x03);
        	bytes = new byte[length % 10];												//나머지 부분의 크기에 맞는 바이트 배열 선언 
        	System.arraycopy(input, length - (length % 10), bytes, 0, length % 10);		//이후 자리부터 나머지 수만큼 복사 
    ...
    public boolean Send(byte[] input, int length) {
        ...
        waitACK();
        if(length > 10) {												//10보다 크면 나눠서 보내야하므로 fragsend
    ...
    public synchronized boolean Receive(byte[] input) {
        ...
        if(tempType == 0) {												//타입이 0이라면 단편화 되지 않고 한번에 받아온 프레임 
        	...
        	if(tempType == 1) {
        		...
        		System.arraycopy(tempBytes, 0, fragBytes, 0, 10);		//fragBytes의 첫 위치에 tempBytes를 넣어줌 
        	}
        	else {
        		tempBytes = RemoveCappHeader(input, input.length);		//헤더 제거된것을 tempbytes로 저장
        		System.arraycopy(tempBytes, 0, fragBytes, (fragCount++) * 10, byte2ToInt(input[0], input[1]));		//fragbytes의 알맞은 위치에 tempbytes를 추가
        		if(tempType == 3) {
