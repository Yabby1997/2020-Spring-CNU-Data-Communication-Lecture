...
public class EthernetLayer implements BaseLayer {
	...
	public boolean Send(byte[] input, int length) {
		if (input == null && length == 0) 										// ack는 input이 없고 length가 0인경우. type을 2로 설정 
			m_sHeader.enet_type = intToByte2(2);
		else if (isBroadcast(m_sHeader.enet_dstaddr.addr)) 						// broadcast는 0xff
			m_sHeader.enet_type = intToByte2(0xff);
		else 																	// nomal는 1
			m_sHeader.enet_type = intToByte2(1);
		
		byte[] bytes = ObjToByte(m_sHeader, input, length);						//인코딩하여 아랫 레이어로 전달 
	...
	public synchronized boolean Receive(byte[] input) {
		...
		if(temp_type == 2) {														//type이 2이면 ack이다. 
			this.GetUpperLayer(0).Receive(null);
		}
		else if(temp_type == 1) {													//type이 1이면 normal이다. 
			if(chkAddr(input) || isBroadcast(input) || !isMyPacket(input)) {		//자신에게 온것이거나 브로드케스트이거나 자신이보낸것이 아니라면
				data = RemoveEthernetHeader(input, input.length);					//헤더를 제거하고 
				this.GetUpperLayer(0).Receive(data);								//상위 레이어에 전달
